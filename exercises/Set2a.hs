...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if length xs > n then drop (lenght xs - 1) xs else xs
...
greet first last 
  | first Nothing = "Hello, " ++ first + "!"
  | first (Just last) = "Hello, " ++ first (Just last) ++ "!"
...
addEithers (Right x) (Left str) = Left str
addEithers (Left str) (Left msg) = Left str
